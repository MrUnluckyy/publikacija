...
export default function AboutVideo({ data }: { data?: AboutData | null }) {
  ...
  const eyebrow = data?.videoEyebrow;
  const heading = data?.videoHeading;
  const body = data?.videoBody;
  const label = data?.videoLabel;
  const ctaLabel = data?.videoCtaLabel;
  const ctaUrl = data?.videoCtaUrl ?? "/book";
